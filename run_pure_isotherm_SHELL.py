...
# See the 'chemical potential to pressure' base code to develop your own calibrations
    chem_pots = [lambda x: 2.3112*numpy.log(x)-38.8328] # dependent on temperature and gas inserted

    # Root directory for gas data.
    data_dir = osp.join('.', 'gases') # gases must be in a folder labeled gases, one down from main folder
    ...
    frame = system.read_lammps('POLYMER_SYSTEM.lmps') # update based on your polymer .lmps file name
    frame.forcefield = 'trappe/amber' # change depending on polymer forcefields you are using

    # Constant for loadings calculations
    molec2mmols_g = 1e+3 / frame.mass # converting molecules -> mmol/g
    ...
    gas_press = numpy.array([0.5,1,1.5,2]) # bar # update prssure based on points you want calculated
